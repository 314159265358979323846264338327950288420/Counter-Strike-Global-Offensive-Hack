...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* display_format, float power )
{
	...
	RenderTextClipped( frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y - 1), value_buf, value_buf_end, NULL, ImVec2( 0.5f, 0.5f ) );
...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	const ImRect check_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( label_size.y + style.FramePadding.y, label_size.y + style.FramePadding.y ) );
	...
	if ( label_size.x > 0.0f )
		RenderText( ImVec2(text_bb.GetTL( ).x, text_bb.GetTL( ).y - 3 /*Fix label position*/), label );
